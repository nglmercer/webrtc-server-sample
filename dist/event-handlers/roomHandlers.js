...
function registerRoomHandlers(socket, listOfRooms, listOfUsers, config, params) {
    ...
    socket.on("open-room", (arg, callback) => {
        ...
        if (listOfRooms[arg.sessionid]?.participants.length) {
            return callback(false, constants_1.CONST_STRINGS.ROOM_NOT_AVAILABLE);
        }
        ...
        (0, roomUtils_1.appendToRoom)(arg.sessionid, socket.userid, maxParticipants, listOfRooms);
    ...
    socket.on("join-room", (arg, callback) => {
        ...
        if (room.password && room.password !== arg.password) {
            return callback(false, constants_1.CONST_STRINGS.INVALID_PASSWORD);
        }
        if (room.participants.length >= room.maxParticipantsAllowed) {
            return callback(false, constants_1.CONST_STRINGS.ROOM_FULL);
        }
        const maxParticipants = parseInt(params.maxParticipantsAllowed || "1000") || 1000;
        (0, roomUtils_1.appendToRoom)(arg.sessionid, socket.userid, maxParticipants, listOfRooms);
        socket.admininfo = { sessionid: arg.sessionid, ...arg };
    ...
    socket.on("get-public-rooms", (identifier, callback) => {
        if (!identifier)
            return callback(null, constants_1.CONST_STRINGS.PUBLIC_IDENTIFIER_MISSING);
    ...
    socket.on("set-password", (password, callback) => {
        ...
        if (!roomid)
            return callback(false, null, constants_1.CONST_STRINGS.DID_NOT_JOIN_ANY_ROOM);
        const room = listOfRooms[roomid];
        if (room && room.owner === socket.userid) {
            ...
            callback(false, roomid, constants_1.CONST_STRINGS.ROOM_PERMISSION_DENIED);
        }
    });
    socket.on("is-valid-password", (password, roomid, callback) => {
        ...
        const room = listOfRooms[roomid];
        if (!room)
            return callback(false, roomid, constants_1.CONST_STRINGS.ROOM_NOT_AVAILABLE);
        if (!room.password)
            return callback(false, roomid, "This room does not have a password.");
        callback(room.password === password, roomid, room.password === password ? "" : constants_1.CONST_STRINGS.INVALID_PASSWORD);
    });
    socket.on("close-entire-session", (callback) => {
        ...
        const room = listOfRooms[roomid];
        if (!room || room.owner !== socket.userid) {
            return callback(false, constants_1.CONST_STRINGS.ROOM_PERMISSION_DENIED);
        }
