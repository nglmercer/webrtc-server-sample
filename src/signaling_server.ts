...
export default function signaling_server(socket: ISocket, config: any = {}) {
  ...
  function onConnection(socket: CustomSocket) {
    ...
    const maxParticipantsAllowed =
      parseInt((params.maxParticipantsAllowed as string) || "1000") || 1000;
    ...
    socket.on(
      ...
      (
        ...
        try {
          ...
          if (
            listOfRooms[roomid] &&
            listOfRooms[roomid].owner === socket.userid
          ) {
    ...
    socket.on(
      "disconnect-with",
      (remoteUserId: string, callback: () => void) => {
        try {
          if (
            listOfUsers[socket.userid] &&
    ...
    socket.on(
      ...
      (callback: (success: boolean, error?: string) => void) => {
        try {
          ...
          if (room.owner !== user.socket.userid)
          ...
          if (
            ...
            extra._room = {
              ...
              isPasswordProtected:
                ...
                listOfRooms[roomid].password.toString().replace(/ /g, "")
                  .length > 0,
            };
        ...
        if (!listOfRooms[roomid]) {
          listOfRooms[roomid] = {
            maxParticipantsAllowed:
              parseInt((params.maxParticipantsAllowed as string) || "1000") ||
              1000,
            owner: userid,
            participants: [userid],
            extra: {},
          ...
          if (socket.userid === listOfRooms[roomid].owner) {
            if (
              ...
              if (firstParticipant) {
                ...
                const newParticipantsList = listOfRooms[
                  roomid
                ].participants.filter((pid) => pid !== socket.userid);
            ...
            const newParticipantsList = listOfRooms[roomid].participants.filter(
              (pid) => pid && pid !== socket.userid && listOfUsers[pid]
            );
            listOfRooms[roomid].participants = newParticipantsList;
          }
        }
    ...
    socket.on(
      ...
      (
        message: any,
        callback: (isPresent: boolean, userid: string) => void
      ) => {
