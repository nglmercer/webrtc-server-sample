 ...
 */
export class UnifiedWebRTCProvider extends EventEmitter implements WebRTCProvider {
  private peerConnection: any = null;
  private dataChannels: Map<string, RTCDataChannel> = new Map();
  private localStreams: MediaStream[] = [];
  private remoteStreams: MediaStream[] = [];
  private config: ExtendedWebRTCConfig;
  ...
  async connect(config?: WebRTCConfig): Promise<void> {
    ...
    // Merge configs
    const finalConfig = { ...this.config, ...config } as ExtendedWebRTCConfig;
  ...
  private async connectReal(config: ExtendedWebRTCConfig): Promise<void> {
    try {
      ...
      // Convert iceServers to node-datachannel format
      // node-datachannel expects an array of strings, not objects
      const iceServers: string[] = [];
      if (config.iceServers) {
        config.iceServers.forEach((server: any) => {
          if (typeof server.urls === 'string') {
            iceServers.push(server.urls);
          } else if (Array.isArray(server.urls)) {
            server.urls.forEach((url: string) => iceServers.push(url));
          }
        });
      }
      ...
      if (iceServers.length === 0) {
        iceServers.push('stun:stun.l.google.com:19302');
      }
      ...
      if (config.debug) {
        ...
        this.logger.debug('Ice servers configured:', iceServers);
      }
    } catch (error: any) {
      throw new Error(`Failed to create REAL WebRTC connection: ${error.message}`);
    }
  }

  private async connectMock(config: ExtendedWebRTCConfig): Promise<void> {
  ...
  private async createRealOffer(): Promise<RTCSessionDescriptionInit> {
    return new Promise((resolve, reject) => {
      ...
      // Try different ways to trigger offer creation and handle description
      try {
        if (typeof this.peerConnection.createOffer === 'function') {
          this.peerConnection.createOffer();
          
          // Set up event listener for description - avoid readonly property issue
          if (this.peerConnection.onLocalDescription) {
            this.peerConnection.onLocalDescription(handleDescription);
          } else if (this.peerConnection.addEventListener) {
            this.peerConnection.addEventListener('localDescription', handleDescription);
          }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.peerConnection.setRemoteDescription && offer.sdp) {
        try {
          ...
          clearTimeout(timeout);
          reject(new Error(`Failed to set remote description: ${error.message}`));
          return;
        }
      }
  ...
  async getStats(): Promise<RTCStatsReport> {
    ...
    return {
      'connection': {
        ...
        type: 'session' as any,
        id: 'connection',
        candidates: this.peerConnection.iceCandidates?.length || 0,
  ...
  getProviderType(): string {
    return this.isReal ? 'unified-webrtc-real' : 'unified-webrtc-mock';
  }
