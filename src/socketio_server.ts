...
wsio.on("connection", (socket: SocketIOLikeSocket) => {
  ...
  socket.on(
    "chat-message",
    (data: { room: string; message: string; username?: string }) => {
      ...
      logger.info(
        `Mensaje de chat en sala ${room} de ${socket.id}: ${message}`,
        {},
      );
    },
  );
  ...
  socket.on(
    "private-message",
    (data: { targetSocketId: string; message: string; username?: string }) => {
      ...
      if (targetUser && targetUser.socket.isAlive()) {
        ...
        logger.info(
          `Mensaje privado de ${socket.id} a ${targetSocketId}: ${message}`,
          {},
        );
      } else {
        socket.emit("error", {
  ...
  socket.on("get-room-users", (data: { room: string }) => {
    const { room } = data;
  ...
  socket.on(
    "global-broadcast",
    (data: { message: string; adminKey?: string }) => {
      ...
      if (adminKey === "admin123") {
        ...
        logger.info(
          `Broadcast global enviado por ${socket.id}: ${message}`,
          {},
        );
      } else {
        socket.emit("error", {
  ...
  // Manejar desconexión
  socket.on("disconnect", (code?: number, reason?: string) => {
...
export function startSocketIOServer(port: number = 8080): void {
  wsio.listen(port, () => {
...
export function stopSocketIOServer(): void {
  wsio.close(() => {
