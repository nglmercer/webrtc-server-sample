...
wsio.on('connection', (socket: SocketIOLikeSocket) => {
  ...
  socket.on('chat-message', (data: { room: string; message: string; username?: string }) => {
  ...
  socket.on('private-message', (data: { targetSocketId: string; message: string; username?: string }) => {
  ...
  socket.on('global-broadcast', (data: { message: string; adminKey?: string }) => {
